...
public class MessageHandlingWorkerPool extends BaseLoggable implements ExceptionHandler<Message> {
	...
	public void publish(Object obj) {
		if (obj instanceof Message) {
			this.publish0((Message) obj);
		} else {
			long sequence = this.ringBuffer.next();
			try {
				MessageRW message = (MessageRW) this.ringBuffer.get(sequence);
				this.messageDeserializer.decode(obj, message);
			} finally {
	...
	private void publish0(Message message) {
		try {
			this.getWorkers()[0].onEvent(message);
		} catch (Exception e) {
			this.handleEventException(e, -1, message);
		}
	}
