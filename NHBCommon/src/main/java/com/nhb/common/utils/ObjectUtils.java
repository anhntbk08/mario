...
public final class ObjectUtils {
	...
	public static <T> Map<String, Setter> findAllClassSetters(Class<T> clazz) {
		...
		if (classSetters.containsKey(clazz)) {
			...
			Field[] fields = getAllFileds(clazz);
	...
	private static Field[] getAllFileds(Class<?> clazz) {
		...
		if (superClass != null && superClass != Object.class) {
			...
			for (int i = 0; i < superFields.length; i++) {
				fields = append(fields, superFields[i]);
			}
			// this.append(fields, ) getAllFileds(superClass);
		}
		return fields;
	}
	...
	private static Method[] getAllMethods(Class<?> clazz) {
		...
		if (superClass != null && superClass != Object.class) {
			...
			for (int i = 0; i < superMethods.length; i++) {
				methods = append(methods, superMethods[i]);
			}
			// this.append(fields, ) getAllFileds(superClass);
		}
		return methods;
	...
	private static final Map<String, Getter> initClassGetters(Class<?> clazz) {
		...
		for (Field field : fields) {
			if (Modifier.isPublic(field.getModifiers()) && !Modifier.isStatic(field.getModifiers())) {
				try {
					// System.out.println("raw field: " + field.getName());
